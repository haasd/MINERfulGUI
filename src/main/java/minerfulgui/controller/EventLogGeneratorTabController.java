...
import minerfulgui.common.EventInfo;
...
import minerfulgui.common.TraceInfo;
...
public class EventLogGeneratorTabController extends AbstractController implements Initializable {
	...
	private final ObservableList<TraceInfo> eventLogList = FXCollections.observableArrayList();

	private final ObservableList<EventInfo> traceInfoList = FXCollections.observableArrayList();
	...
	private Boolean creatingLog = false;
	...
	private void generateLog(Boolean save) {
		...
		if (save && logMak == null) {
			...
			if (saveFile != null) {
				String extension = saveFile.getName().substring(saveFile.getName().lastIndexOf(".") + 1,
				...
				if (!creatingLog) {
					...
					createLog.setOnFailed(new EventHandler<WorkerStateEvent>() {
						@Override
						public void handle(WorkerStateEvent event) {
							MinerfulGuiUtil.displayAlert("Error", "Error during log creation!",
									"An error occured during the creation of the event log", AlertType.ERROR);
							creatingLog = false;
						}
					});
				}
	...
	private Boolean checkInput() {
		...
		if (minEvents > maxEvents) {
			MinerfulGuiUtil.displayAlert("Input Error", "Input Error",
					"Maximum number of events per trace has to be less than the minimum number of events per trace!",
					AlertType.ERROR);
			return false;
